...
from .config import PANEL_NAME, VERSION, AUTHOR, REPORT_FILE
...
class KadoPanelMain(Screen):
    ...
    <screen name="KadoPanelMain" position="center,center" size="1000,650" title="Kado Panel">
        ...
        <widget name="title" position="35,25" size="720,45" font="Regular;34" foregroundColor="#d4af37" backgroundColor="#101010" transparent="1" />
        <widget name="subtitle" position="35,70" size="720,35" font="Regular;22" foregroundColor="#ffffff" backgroundColor="#101010" transparent="1" />
    ...
    def __init__(self, session):
        ...
        Logger.write("Kado Panel v0.2.0 Started")
    ...
    def ok(self):
        ...
        if current == "Health Check":
            ...
            self["status"].setText("Kado AI Advisor\n\n%s" % AIAdvisor().advise(health, neo))

        elif current == "System Information":
            info = SystemInfo().get_all()
            lines = ["System Information", ""]
            for key in ["receiver", "image", "image_version", "python", "machine", "kernel", "flash_free_mb", "mem_free_mb", "tmp_free_mb", "internet"]:
            ...
            self["status"].setText("Smart Install Preview\n\nThis module will start in v0.3.0 Alpha.\n\nHealth Check will always run before install.")
